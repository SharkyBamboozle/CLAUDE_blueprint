...
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
mkdir -p "$TMP/docs/project/decisions" "$TMP/docs/project/process" "$TMP/.claude/working"
DECIDED="docs/project/decisions/adr-0001-fixture.md"
PROPOSED="docs/project/decisions/adr-0007-fixture.md"
...
printf '# Contributing\n' >"$TMP/docs/project/process/contributing.md"
printf '# Decisions\n' >"$TMP/docs/project/decisions/index.md"
...
expect 2 "create new ADR stamped ✅ -> blocked" \
  Write "{\"file_path\":$(J docs/project/decisions/adr-0099-new.md),\"content\":$(J '# ADR-0099
- **Status:** ✅ Decided')}"
expect 2 "off-format ✅ status line (no bullet) -> blocked" \
  Write "{\"file_path\":$(J docs/project/decisions/adr-0099-new.md),\"content\":$(J '**Status:** ✅ Decided')}"
...
expect 2 "Decided ADR via ../ path (normalization) -> blocked" \
  Edit "{\"file_path\":$(J docs/project/decisions/../decisions/adr-0001-fixture.md)}"
expect 2 "git rm Decided ADR (delete) -> blocked" \
  Bash "{\"command\":$(J "git rm $DECIDED")}"
expect 2 "git mv Decided ADR (rename) -> blocked" \
  Bash "{\"command\":$(J "git mv $DECIDED docs/project/decisions/adr-0001-renamed.md")}"
...
expect 2 "git -C docs/project/decisions rm bare Decided name -> blocked (path resolves against -C)" \
  Bash "{\"command\":$(J 'git -C docs/project/decisions rm adr-0001-fixture.md')}"
expect 0 "git -C docs/project/decisions rm bare Proposed name -> allowed (partner)" \
  Bash "{\"command\":$(J 'git -C docs/project/decisions rm adr-0007-fixture.md')}"
...
expect 2 "top-level backtick git rm of Decided ADR -> blocked" \
  Bash "{\"command\":$(J "\`git rm $DECIDED\`")}"
expect 2 "git rm -r docs/project/decisions (parent dir) -> blocked" \
  Bash "{\"command\":$(J 'git rm -r docs/project/decisions')}"
...
expect 2 "git rm -r :/* (root glob) -> blocked" \
  Bash "{\"command\":$(J 'git rm -r :/*')}"
expect 2 "git rm docs/project/deci* (glob prefix reaching the ADR dir) -> blocked" \
  Bash "{\"command\":$(J 'git rm -r docs/project/deci*')}"
expect 2 "git rm docs/proj* (glob prefix under docs) -> blocked" \
  Bash "{\"command\":$(J 'git rm -r docs/proj*')}"
...
expect 2 "git rm brace-expansion incl. Decided ADR -> blocked" \
  Bash "{\"command\":$(J "git rm docs/project/decisions/adr-{0001-fixture,0007-fixture}.md")}"
...
expect 2 "coreutils rm -rf docs/project/decisions (parent dir) -> blocked" \
  Bash "{\"command\":$(J 'rm -rf docs/project/decisions')}"
...
expect 2 "coreutils rm brace-expansion incl. Decided ADR -> blocked" \
  Bash "{\"command\":$(J "rm docs/project/decisions/adr-{0001-fixture,0007-fixture}.md")}"
expect 2 "git rm brace-RANGE incl. Decided ADR -> blocked" \
  Bash "{\"command\":$(J 'git rm docs/project/decisions/adr-{0001..0001}-fixture.md')}"
expect 2 "git rm over-cap brace-range under decisions -> blocked (scope scan)" \
  Bash "{\"command\":$(J 'git rm docs/project/decisions/adr-000{1..2000}-fixture.md')}"
expect 2 "git rm DESCENDING brace-range past cap -> blocked (scope scan)" \
  Bash "{\"command\":$(J 'git rm docs/project/decisions/adr-{0065..0001}-fixture.md')}"
expect 0 "over-cap brace-range OUTSIDE the ADR tree -> allowed (no false-block)" \
  Bash "{\"command\":$(J 'rm docs/project/process/note-{1..2000}.md')}"

# ---- cd context is tracked across && / ; lists -------------------------
expect 2 "cd docs/project/decisions && git rm bare Decided name -> blocked" \
  Bash "{\"command\":$(J 'cd docs/project/decisions && git rm adr-0001-fixture.md')}"
expect 2 "cd docs/project/decisions && rm bare Decided name -> blocked" \
  Bash "{\"command\":$(J 'cd docs/project/decisions && rm adr-0001-fixture.md')}"
expect 2 "cd docs/project && rm -r decisions (parent dir via cd) -> blocked" \
  Bash "{\"command\":$(J 'cd docs/project && rm -r decisions')}"
expect 0 "cd docs/project/process && rm contributing.md (not an ADR) -> allowed (partner)" \
  Bash "{\"command\":$(J 'cd docs/project/process && rm contributing.md')}"
...
expect 2 "(cd sub) ; git rm <full-path Decided> -> blocked (no subshell-cd leak miss)" \
  Bash "{\"command\":$(J '(cd sub) ; git rm docs/project/decisions/adr-0001-fixture.md')}"
expect 2 "(cd docs/project) ; rm decisions/<Decided> -> blocked (cd-context catches it)" \
  Bash "{\"command\":$(J '(cd docs/project) ; rm decisions/adr-0001-fixture.md')}"
expect 0 "(cd docs/project/decisions) ; git rm docs/project/process/<non-ADR> -> allowed (partner)" \
  Bash "{\"command\":$(J '(cd docs/project/decisions) ; git rm docs/project/process/contributing.md')}"
...
expect 0 "coreutils rm of a non-ADR file -> allowed" \
  Bash "{\"command\":$(J 'rm docs/project/process/contributing.md')}"
expect 0 "coreutils mv INTO docs/project/decisions -> allowed (creates, not destroys)" \
  Bash "{\"command\":$(J 'mv docs/project/process/contributing.md docs/project/decisions/')}"
expect 2 "git mv docs/project/decisions archive (parent dir rename) -> blocked" \
  Bash "{\"command\":$(J 'git mv docs/project/decisions archive')}"
expect 0 "git rm -r docs/project/process (no ADRs beneath) -> allowed (partner)" \
  Bash "{\"command\":$(J 'git rm -r docs/project/process')}"
expect 0 "git mv INTO docs/project/decisions -> allowed (creates, not destroys)" \
  Bash "{\"command\":$(J 'git mv docs/project/process/contributing.md docs/project/decisions/')}"
...
expect 0 "registry index.md (not an ADR) -> allowed" \
  Edit "{\"file_path\":$(J docs/project/decisions/index.md)}"
expect 0 "unrelated docs page -> allowed" \
  Edit "{\"file_path\":$(J docs/project/process/contributing.md)}"
...
expect 0 "git -C . rm Decided ADR with fresh token -> allowed" \
  Bash "{\"command\":$(J "git -C . rm $DECIDED")}"
expect 0 "git rm -r docs/project/decisions, its only Decided ADR unlocked -> allowed" \
  Bash "{\"command\":$(J 'git rm -r docs/project/decisions')}"
